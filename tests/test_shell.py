...
def _make_config(signals=None):
    config = MagicMock()
    config.signals = signals or {}
    return config
...
def test_dispatch_signal():
    ...
    config = _make_config({"deploy": sig})
    push = MagicMock()

    with patch("propagate_app.shell.send_signal") as mock_send:
        _cmd_signal("deploy url:http://example.com", config, push)
        mock_send.assert_called_once_with(push, "deploy", {"url": "http://example.com"})


def test_dispatch_signal_unknown(capsys):
    config = _make_config({"deploy": _signal_config("deploy", [])})
    push = MagicMock()

    _cmd_signal("unknown", config, push)
    out = capsys.readouterr().out
    assert "not defined in config" in out
...
def test_dispatch_signal_missing_required(capsys):
    ...
    _cmd_signal("deploy", config, push)
    out = capsys.readouterr().out
...
def test_dispatch_resume():
    ...
    with patch("propagate_app.shell.send_command") as mock_cmd:
        _cmd_resume(push)
        mock_cmd.assert_called_once_with(push, "resume")
...
def test_dispatch_quit_returns_sentinel():
    ...
    push = MagicMock()
    buf = collections.deque(maxlen=500)
    assert _dispatch("/quit", config, push, buf) is _QUIT
...
def test_dispatch_exit_returns_sentinel():
    ...
    push = MagicMock()
    buf = collections.deque(maxlen=500)
    assert _dispatch("/exit", config, push, buf) is _QUIT
...
def test_event_listener_log_buffered():
    ...
    with patch("propagate_app.shell.receive_event") as mock_recv:
        ...
        _event_listener(MagicMock(), log_buffer, stop)
