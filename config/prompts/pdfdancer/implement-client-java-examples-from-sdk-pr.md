...
```bash
SDK_PR_NUMBER="$(propagate context get :client-java-pr-number --task implement-client-java | xargs)"
SDK_SHA="$(gh pr view "$SDK_PR_NUMBER" --repo MenschMachine/pdfdancer-client-java --json commits --jq '.commits[-1].oid')"
```

Use that commit in the dependency configuration:
...
If the project uses centralized repository management, also update `settings.gradle.kts`:
