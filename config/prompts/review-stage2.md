...
You are reviewing the stage 2 implementation of Propagate. The repository started from stage 1 and should now include a local context bag and prompt augmentation during `run`.

## Review goals

1. Verify stage 1 behavior still works:
...
2. Verify stage 2 additions work:
   - `propagate context set <key> <value>`
   - `propagate context get <key>`
   - `.propagate-context/<key>` storage
   - deterministic context injection into prompts
3. Verify the bootstrap chain was advanced:
   - `config/propagate.yaml` should now target building stage 3
   - `config/prompts/design-stage3.md`, `config/prompts/implement-stage3.md`, and `config/prompts/review-stage3.md` should exist and describe adding hooks and context sources
4. Fix problems directly if you find them.

## Review checklist

- Reject unsafe context keys.
- Missing context keys fail clearly.
- `context get` writes the stored value to stdout, not logs.
- `run` still works when `.propagate-context` does not exist.
- Context values are loaded in sorted order.
- Prompt augmentation format is readable and deterministic.
- Existing error handling and logging remain clear.
- The implementation stays within stage 2 scope and does not partially add hooks or git behavior.

## Full Propagate vision

Propagate is a staged, self-hosting orchestrator for AI agent tasks. The end state includes:

- Config-driven executions across repositories
- Context bag scopes for runtime data
- Hooks and named context sources
- Git automation after sub-tasks
- Signal-driven triggering and propagation
- Multi-repo DAG execution

The current review is only about stage 2. Stage 3 is where hooks and context sources arrive.

## Current stage 1 code baseline

This is the exact pre-implementation `propagate.py` baseline that the stage 2 work started from:
...
- Inspect the current working tree, not just the intended design.
- Run targeted checks if useful.
- Make fixes directly rather than only reporting them.
